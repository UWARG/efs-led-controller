/*!
 * @file SlimAdafruit_Neopixel.cpp
 *
...
/*!
  ...
  @param   pixels Pointer to an array of uint_8t's of at least 3n (if
              LEDs use 24-bit packetss) or 4n (if LEDs use 32-bit
              packets). It is up to you to allocate this array properly,
              if it is too short it will cause undefined behavior (and
              maybe rave lights).
  @param   p  Arduino pin number which will drive the NeoPixel data in.
  @param   t  Pixel type -- add together NEO_* constants defined in
              SlimAdafruit_Neo.h, for example NEO_GRB+NEO_KHZ800 for
              NeoPixels expecting an 800 KHz (vs 400 KHz) data stream
              with color bytes expressed in green, red, blue order per
              pixel.
  @return  SlimAdafruit_Neo object. Call the begin() function before use.
*/
SlimAdafruit_NeoPixel::SlimAdafruit_NeoPixel(uint16_t n, uint8_t *pixels, int16_t p, neoPixelType t)
...
void SlimAdafruit_Neo::rp2040Init(uint8_t pin, bool is800KHz)
{
...
// Not a user API
void  SlimAdafruit_Neo::rp2040Show(uint8_t pin, uint8_t *pixels, uint32_t numBytes, bool is800KHz)
{
  if (this->init)
  {
...
*/
void SlimAdafruit_NeoPixel::show(void) {
...
*/
void SlimAdafruit_NeoPixel::setPin(int16_t p) {
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
#if defined(__AVR__)
...
*/
void SlimAdafruit_NeoPixel::setPixelColor(uint16_t n, uint8_t r, uint8_t g,
                                      uint8_t b) {
...
*/
void SlimAdafruit_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
...
*/
void SlimAdafruit_NeoPixel::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t SlimAdafruit_NeoPixel::getPixelColor(uint16_t n) const {
  if (n >= numLEDs)
...
/*!
  ...
  @note    This function is declared static in the class so it can be called
           ...
           in the code yet). Use SlimAdafruit_Neo::str2order().
*/
neoPixelType SlimAdafruit_NeoPixel::str2order(const char *v) {
